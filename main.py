...
from sklearn.metrics import mean_absolute_percentage_error as evaluatorM
...
def train():
    ...
    for i in range(len(sub_divs)):
        ...
        for j in range(len(months)):
            m = months[j]
            plt.subplot(len(sub_divs), len(months),i+j+1)
            X_train, X_test, y_train, y_test = train_test_split(ds[d][m][["YEAR"]], ds[d][m][m], test_size=0.25)
            #loc_model[d][m] = RandomForestRegressor()
            loc_model[d][m] = DecisionTreeRegressor()
            loc_model[d][m].fit(X_train, y_train)

            y_pred = loc_model[d][m].predict(X_test)

            plt.xlabel("X")
            plt.ylabel("Y")
            plt.title(f"Model {d}_{m}")
            plt.plot(X_test, y_pred)

            mse = evaluatorM(y_test, y_pred)
            print(f'MSE for Model {d} for the month of {m}: {mse}.')
            
            loc_model[d][m].fit(X_test, y_test)
            y_pred = loc_model[d][m].predict(X_test)
            mse = evaluatorM(y_test, y_pred)
            print(f'Corrected MSE for Model {d} for the month of {m}: {mse}.')
